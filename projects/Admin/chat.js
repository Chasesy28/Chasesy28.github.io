...
function extractMessageAuthorEmail(message) {
  return (
    normalizeText(message?.sender_email) || normalizeText(message?.email) || ""
  );
}
...
function getSupabaseCallbackState() {
  ...
  const hasCallbackMarkers =
    hasCode ||
    hasAccessToken ||
    searchParams.has("state") ||
    Boolean(searchError || hashError);
...
function renderSessionSummary() {
  ...
  els.sessionExpires.textContent = session
    ? String(getSessionExpirationMinutes(session) ?? 0)
    : "-";
  els.memberId.textContent = state.memberRecord
    ? (extractMemberId(state.memberRecord) ?? "-")
    : "-";
}
...
function renderMessage(message) {
  ...
  const authorLabel = isSelf
    ? normalizeText(state.currentUser?.email) ||
      normalizeText(state.memberRecord?.email) ||
      "member"
...
function renderMessages(messages) {
  ...
  if (!messages.length) {
    renderEmptyState(
      "No messages yet. Once someone in chat_members posts here, they will appear live in this stream.",
    );
    return;
  }
...
async function resolveAccess(callbackState = getSupabaseCallbackState()) {
  if (!isSupabaseConfigured()) {
    ...
    setNotice(
      getSupabaseConfigError() ||
        "Supabase credentials are missing for this page.",
      "error",
    );
  ...
  state.currentUser =
    state.session?.user ?? (await getCurrentSupabaseUser().catch(() => null));
  ...
  if (!state.session || !state.currentUser) {
    ...
    setNotice(
      "Sign in with Google to unlock chat, then your email must exist in chat_members.",
      "info",
    );
  ...
  if (!matchedMember) {
    ...
    setNotice(
      "Your session is valid, but your email does not appear in chat_members.",
      "error",
    );
...
function setMessagePaneForLockedState() {
  ...
  renderEmptyState(
    "Chat is locked until you sign in and your email matches an entry in chat_members.",
  );
}

async function loadMessages() {
  if (!supabase || state.loadingMessages || state.access !== "authorized")
    return;
...
async function clearAllMessages() {
  ...
  try {
    ...
    setNotice(
      error instanceof Error ? error.message : "Unable to clear messages.",
      "error",
    );
  } finally {
...
function applyIncomingMessage(messageRow) {
  ...
  if (
    normalizedId &&
    state.messages.some(
      (message) => String(extractMessageId(message)) === String(normalizedId),
    )
  ) {
    return;
  }
...
async function tryInsertMessage(content) {
  if (!supabase || !state.memberRecord || !state.currentUser)
...
async function bootstrap() {
  ...
  if (!isSupabaseConfigured()) {
    ...
    setBadge("error", "Supabase missing");
    setNotice(
      getSupabaseConfigError() || "Add Supabase credentials to enable chat.",
      "error",
    );
    setConnectionState("Unavailable");
    setComposerEnabled(false);
